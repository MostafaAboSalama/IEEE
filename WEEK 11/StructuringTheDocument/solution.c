...
struct document get_document(char* text) {
    ...
    for (int p=0; p<5; p++) {
        doc.data[p].data = malloc(sizeof(struct sentence) * 10);
        doc.data[p].sentence_count = 0;
        for (int s = 0; s < 10; s++) {
          doc.data[p].data[s].data = malloc(sizeof(struct word) * 100);
          doc.data[p].data[s].word_count = 0;
          for (int w = 0; w < 100; w++){
              doc.data[p].data[s].data[w].data = malloc(sizeof(char) * 200);
            }
        }
    }
